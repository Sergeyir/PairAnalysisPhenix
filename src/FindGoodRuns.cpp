...
int main(int argc, char **argv)
{
   if (argc < 2 || argc > 4) 
   {
      CppTools::PrintError("Expected 1-3 parameters while " + std::to_string(argc - 1) + " "\
                           ...
                           "runName multThreshold=2. chi2NDFThreshold=3.");
   }
...
void FindGoodRuns::CheckRunsByMultiplicityAndFindReferenceRun()
{
   ...
   averageMultiplicity /= static_cast<double>(goodRuns.size());
   averageChargeRatio /= static_cast<double>(goodRuns.size());

   CppTools::Print(averageMultiplicity, averageChargeRatio);


   /*
   for (const auto& fileName : goodRunFileNames)
   {
