...
void DrawSpectra() 
{
	...
	SpectraPainter spectra = SpectraPainter(Par.particle.name + ",  " + Par.CType.name, 
		0.7, 6.1, "p_{T}, GeV/c", 
		"1/(2 #pi) 1/p_{T} d^{2} N/dp_{T}/dy, (GeV/c)^{-2}");
	...
	Print("mkdir -p ../output/Tables/Spectra/" + Par.run + "/" + Par.runnum);
	...
	spectra.PrintSpectra("../output/Tables/Spectra/" + 
	...
	spectra.AddGraph("../ext/Spectra/pp200/KStar892.txt",
		...
		"p+p", 0, 1./Par.pp_sigma, Par.particle.mass, 57, kGray+2);

   spectra.PrintFitFuncsParameters("../output/Tables/Spectra/" + Par.run + "/" + Par.runnum);

	spectra.Draw("../output/InvM/" + Par.run + "/" + 
